...
// Petits composants locaux a cette page uniquement.
// NOTE : ils etaient utilises dans le fichier original (<Dot/>, <Bar/>)
// sans jamais etre definis nulle part — un bug latent preexistant qui
// aurait fait planter cette page a l'execution ("Dot is not defined").
// On les definit ici en se basant sur leurs props d'usage (on=bool, val/color).
function Dot({ on }) {
...
// MetricRow local a cette page (different du MetricRow partage dans
// components/Common.jsx) — gere aussi les valeurs booleennes (OK/FAIL)
// et les valeurs texte (ex: "8/18 GB"), pas seulement les pourcentages.
function InfraMetricRow({ label, value, unit='', warn=75, crit=90, isTemp=false, isBool=null, isOk=null }) {
  const num = parseFloat(value) || 0
  let color = C.green
  if (isBool !== null) {
    color = isBool ? C.green : C.red
  } else if (isOk !== null) {
  ...
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom:`1px solid ${C.border}20` }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        ...
        <span style={{ fontSize:12, fontWeight:700, color, fontFamily:'JetBrains Mono,monospace', minWidth:60, textAlign:'right' }}>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div>
          <h2 style={{ fontSize:18, fontWeight:700, color:C.text, marginBottom:4 }}>Infrastructure</h2>
          <div style={{ fontSize:12, color:C.sub }}>{noeuds.length} hypervisor{noeuds.length>1?'s':''} · {vms.length} VM{vms.length>1?'s':''} · Click any resource to see all metrics</div>
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns: sel ? '1fr 1.4fr' : '1fr', gap:16 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          {/* Nœuds */}
          <div style={{ fontSize:10, color:C.muted, fontFamily:'JetBrains Mono,monospace', letterSpacing:'0.1em', padding:'4px 0' }}>HYPERVISORS</div>
          {noeuds.map((n, i) => {
              ...
              >
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                  <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                    ...
                    <div style={{ fontSize:11, color:C.muted, fontFamily:'JetBrains Mono,monospace', display:'flex', alignItems:'center', gap:6 }}>
                      ...
                      {isOnline ? (
                        <>
                          ...
                          {(n.uptime_h??0) > 0 && (
                            <>
                              <span style={{ color:C.border }}>·</span>
                              <span>up {fmtUptime(n.uptime_h)}</span>
                            </>
                          )}
                        </>
                      ) : (
                        <>
                          <span style={{ color:C.border }}>·</span>
                          <span style={{ color:C.red }}>Unreachable</span>
                        </>
                      )}
                    </div>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    {!tempOk && <span style={{ fontSize:11, color:C.orange }}>🌡 {n.cpu_temp_max_c}°C</span>}
                    {!(n.smart_ok??true) && <span style={{ fontSize:11, color:C.red }}>⚠ DISK</span>}
                    {!(n.corosync_ok??true) && <span style={{ fontSize:11, color:C.red }}>⚠ QUORUM</span>}
                    <Dot on={isOnline}/>
                    <span style={{ fontSize:11, color:isOnline?C.green:C.red, fontWeight:700 }}>{isOnline?'Online':'Offline'}</span>
                  </div>
                </div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
                  <div>
                    <div style={{ fontSize:10, color:C.muted, marginBottom:3 }}>CPU</div>
                    <Bar val={n.cpu_pct??0} color={cpuColor}/>
                    <div style={{ fontSize:11, color:cpuColor, fontFamily:'JetBrains Mono,monospace', marginTop:3 }}>{n.cpu_pct??0}%</div>
                  </div>
                  <div>
                    <div style={{ fontSize:10, color:C.muted, marginBottom:3 }}>RAM</div>
                    <Bar val={n.ram_pct??0} color={ramColor}/>
                    <div style={{ fontSize:11, color:ramColor, fontFamily:'JetBrains Mono,monospace', marginTop:3 }}>{n.ram_used_gb??0}/{n.ram_total_gb??0} GB</div>
                  </div>
                  <div>
                    <div style={{ fontSize:10, color:C.muted, marginBottom:3 }}>DISK</div>
                    <Bar val={n.disk_pct??0} color={(n.disk_pct??0)>=90?C.red:(n.disk_pct??0)>=80?C.yellow:C.green}/>
                    <div style={{ fontSize:11, color:C.sub, fontFamily:'JetBrains Mono,monospace', marginTop:3 }}>{n.disk_pct??0}%</div>
                  </div>
                </div>
                {/* Indicateurs avancés */}
                <div style={{ display:'flex', gap:12, marginTop:10, flexWrap:'wrap' }}>
                  {(n.swap_pct??0)>0 && <span style={{ fontSize:10, color:(n.swap_pct??0)>50?C.orange:C.muted }}>Swap {n.swap_pct??0}%</span>}
                  {(n.cpu_iowait_pct??0)>0 && <span style={{ fontSize:10, color:(n.cpu_iowait_pct??0)>15?C.orange:C.muted }}>I/O wait {n.cpu_iowait_pct??0}%</span>}
                  {(n.net_in_mbps??0)>0 && <span style={{ fontSize:10, color:C.muted }}>↓{n.net_in_mbps??0} ↑{n.net_out_mbps??0} MB/s</span>}
                  {(n.zfs_arc_hit_rate??0)>0 && <span style={{ fontSize:10, color:(n.zfs_arc_hit_rate??0)<70?C.orange:C.muted }}>ZFS {n.zfs_arc_hit_rate??0}%</span>}
                  {(n.uptime_h??0)>0 && <span style={{ fontSize:10, color:C.muted }}>Up {fmtUptime(n.uptime_h)}</span>}
                </div>
              </div>
            )
          })}
          ...
          {vms.length > 0 && (
            <>
              <div style={{ fontSize:10, color:C.muted, fontFamily:'JetBrains Mono,monospace', letterSpacing:'0.1em', padding:'4px 0', marginTop:4 }}>VIRTUAL MACHINES</div>
              {vms.map((v, i) => {
                ...
                return (
                  ...
                  >
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                      <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                        <div style={{ fontSize:14, fontWeight:700, color:C.text }}>{v.nom||v.name||`VM-${v.vmid}`}</div>
                        <div style={{ fontSize:11, color:C.muted, fontFamily:'JetBrains Mono,monospace', display:'flex', alignItems:'center', gap:6 }}>
                          ...
                          {isRunning && (v.uptime_h??0) > 0 && (
                            <>
                              <span style={{ color:C.border }}>·</span>
                              <span>up {fmtUptime(v.uptime_h)}</span>
                            </>
                          )}
                        </div>
                      </div>
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        <Dot on={isRunning}/>
                        <span style={{ fontSize:11, color:isRunning?C.green:C.muted, fontWeight:700 }}>{isRunning?'Running':'Stopped'}</span>
                      </div>
                    </div>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8 }}>
                      <div>
                        ...
                        <div style={{ fontSize:11, color:cpuColor, fontFamily:'JetBrains Mono,monospace', marginTop:3 }}>{v.cpu_pct??0}%</div>
                      </div>
                      <div>
                        <div style={{ fontSize:10, color:C.muted, marginBottom:3 }}>RAM</div>
                        <Bar val={v.ram_pct??0} color={ramColor}/>
                        <div style={{ fontSize:11, color:ramColor, fontFamily:'JetBrains Mono,monospace', marginTop:3 }}>{v.ram_used_gb??0}/{v.maxmem_gb??v.ram_total_gb??'?'} GB</div>
                      </div>
                      <div>
                        <div style={{ fontSize:10, color:C.muted, marginBottom:3 }}>DISK</div>
                        <Bar val={v.disk_pct??0} color={(v.disk_pct??0)>=90?C.red:C.green}/>
                        <div style={{ fontSize:11, color:C.sub, fontFamily:'JetBrains Mono,monospace', marginTop:3 }}>{v.disk_pct??0}%</div>
                      </div>
                    </div>
                  </div>
                )
        ...
        {/* Panneau détail — toutes les ressources */}
          ...
          <Card style={{ padding:'16px 20px', position:'sticky', top:20, maxHeight:'85vh', overflowY:'auto' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
              ...
              <button onClick={()=>setSel(null)}
                style={{ background:'transparent', border:`1px solid ${C.border}`, color:C.muted, borderRadius:6, padding:'4px 10px', cursor:'pointer', fontSize:11 }}>
                ✕ Close
              </button>
            </div>
